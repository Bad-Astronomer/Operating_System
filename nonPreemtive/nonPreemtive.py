...
def shortestJobFirst():
    ...
    for clock in range(totalTime):
        ...
        if not running:
            waiting = sorted(waiting, key=lambda process: process.serviceTime)            
        waiting[0].update()
    waiting[0].finishTime = clock + 1
    waiting.pop(0)
    
    calc("Shortest Job First")
...
def priority():
    global processes, gantt
    waiting, remaining = [], [process for process in processes]
    remaining = sorted(remaining, key=lambda process: process.priority)
